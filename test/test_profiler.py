...
class TestAsyncGenException:
    ...
    def test_9(self) -> None:
        @decorate
        def sync_gen() -> Generator[Literal[10, 20, 30]]:
            yield 10
            g = _gen_copy(1, 2)
            g.send(None)
            try:
        ...
        @decorate
        async def async_gen() -> AsyncGenerator[Literal[10, 20, 30]]:
            yield 10
            ag = _agen_copy(1, 2)
            await ag.asend(None)
            try:
    ...
    def test_10(self) -> None:
        @decorate
        async def agen() -> AsyncGenerator[Literal[123]]:
            with pytest.raises(
                RuntimeError,
