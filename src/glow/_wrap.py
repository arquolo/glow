...
class _FutureLike[R](_Proxy[Awaitable[R]]):
    def __await__(self) -> Generator[Any, Any, R]:
        ...
        if gen is self.__wrapped__:
            assert isinstance(self, _Generator)
            return self
...
async def _coroutine[R](coro: Coro[R], wrapper: _Wrapper) -> R:
...
def _wrap[T](r: T, wrapper: _Wrapper) -> T:  # noqa: C901,PLR0911
    ...
    match r:
        ...
        case types.AsyncGeneratorType() if _OP_FUNC:
            return _asyncgen(r, wrapper)  # async generator functions
        case AsyncGenerator():
            return _AsyncGenerator(r, wrapper)  # user's async generators

        # __aiter__, __anext__
        case AsyncIterator():
            return _AsyncIterator(r, wrapper)  # user's async iterators

        # __await__, send, throw, close
        case types.CoroutineType() if _OP_FUNC:
            cr = _coroutine(r, wrapper)
            weakref.finalize(cr, r.close)
            return cast('T', cr)  # coroutine functions
        case Coroutine():
            return _Coroutine(r, wrapper)  # user's coroutines

        # __await__
        case Awaitable():
            return _FutureLike(r, wrapper)  # Future-like from function
