...
class _Optimizer(optimizer.Optimizer):
    ...
    @abc.abstractmethod
    def _do_step(
        self, p: torch.Tensor, state: dict[str, torch.Tensor], **group
    ) -> None:
        raise NotImplementedError


def _apply_weight_decay(p: torch.Tensor, lr: float, weight_decay: float, **kwargs):
    if weight_decay:
...
class SGDW(_Optimizer):
    ...
    def __init__(
        ...
        for k, v in zip(
            ["lr", "momentum", "weight_decay"], [lr, momentum, weight_decay]
        ):
            if v < 0:
    ...
    def _do_step(p: torch.Tensor, state: dict[str, torch.Tensor], **group):
        ...
        if momentum := group["momentum"]:
            ...
            grad = p.grad.add(avg, alpha=momentum) if group["nesterov"] else avg
        else:
            grad = p.grad
...
class AdamW(_Optimizer):
    ...
    def _do_step(p: torch.Tensor, state: dict[str, torch.Tensor], **group):
        ...
        for avg_, grad, beta in zip(
            [avg, avg_sq], [p.grad, p.grad * p.grad], group["betas"]
        ):
...
class RAdam(_Optimizer):
    ...
    def _prepare_group(self, **group) -> dict[str, Any]:
        ...
        return {
            "is_tractable": True,
            "step_size": group["lr"] * (correction2 * factor) ** 0.5 / correction1,
        }
...
class Optimizer:
    ...
    def state_dict(self):
        ...
        state = {
            (mappings[id(k)] if isinstance(k, torch.Tensor) else k): v
    ...
    def load_state_dict(self, state_dict):
        ...
        state = dict(
            [
                (
                    (param, cast(param, v))
                    if (param := id_map.get(k)) is not None
                    else (k, v)
                )
